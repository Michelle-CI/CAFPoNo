...
def perm_to_dag_sequential(X, perm, ci_test, alpha, n_jobs=1, verbose=False, **kwargs):
    N, d = X.shape
...
def cnf_pruning(X, perm, threshold=0.1, normalize=True, hidden_sizes=4, n_components=64, lr=0.1, l1=1e-6, l2=5e-5, batch_size=512, max_epochs=100, random_state=0, verbose=False, **kwargs):
...
def cam_pruning(X, perm, threshold=0.1, verbose=False, **kwargs):
    N, d = X.shape
    A = np.ones((d, d))
    for i in range(1, d):
