...
from src.methods.modules.causa.utils import TensorDataLoader
from sklearn.preprocessing import StandardScaler, SplineTransformer

from src.methods.modules.causa.hsic import HSIC
from src.methods.modules.causa.het_ridge import convex_fgls
from src.methods.modules.causa.ml import map_optimization
...
def het_fit_nn(x, y, n_steps=None, seed=711, device='cpu'):
    ...
    @torch.no_grad()
    def f(x_):
        x_ = torch.from_numpy(x_[:, np.newaxis]).double().to(device=device)
        f = model(x_)
        eta_2 =  - f[:, 1] / 2
        scale = torch.sqrt(- 0.5 / eta_2)
        loc = - 0.5 * f[:, 0] / eta_2
...
def loci(x, y, independence_test=True, neural_network=True, return_function=False, n_steps=None):
    ...
    if neural_network:
        log_lik_forward, f_forward = het_fit_nn(x, y, n_steps, device='cuda')
        log_lik_reverse, f_reverse = het_fit_nn(y, x, n_steps, device='cuda')
    else:
        log_lik_forward, f_forward = het_fit_convex(x, y, n_steps, device='cuda')
        log_lik_reverse, f_reverse = het_fit_convex(y, x, n_steps, device='cuda')
    ...
    if independence_test:
        ...
        return indep_reverse, indep_forward
        # score = indep_reverse - indep_forward
    else:
        # score = log_lik_forward - log_lik_reverse
        return log_lik_forward, log_lik_reverse
