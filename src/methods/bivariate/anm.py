...
from src.methods.modules.nets import MLP1layer, MLP4
    ...
    **Multivariate case**: This class also supports interventions in a multivariate case, where we suppose that
    ...
    def _get_regressor(self):
        if self.method.lower() == 'gp':
            return GaussianProcessRegressor(random_state=self.config.seed)
...
def ANM(X, Y, args=None):
    model = ANMModel(config=args)
    X, Y = map(standardize, (X, Y))
    X, Y = map(lambda x: x.reshape(-1, 1), (X, Y))
    score_XY = model.anm_score(Y, X)
    score_YX = model.anm_score(X, Y)
