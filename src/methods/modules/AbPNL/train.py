...
from src.methods.modules.AbPNL.module import AbPNLModel, NtoOneDataset
import src.methods.modules.AbPNL.stats as stats
...
class AbPNLTrainer(object):
    ...
    def doit_bivariate(self, x_train, x_test):
        self._x_train = x_train
        self._x_test = x_test
        self._variables = list(range(2))
        logs = self._train_bivariate()
        self._select_var(logs, "loss/test","loss_ind/test", np.median)
        score_XY = -self._log_stats[0][1]
        score_YX = -self._log_stats[1][1]
    ...
    def doit(
        ...
        self._x_train = x_train
        self._x_test = x_test
        num_variables = len(x_train[0])
        self._variables = list(range(num_variables))

        os.makedirs(self._params["logdir"], exist_ok=True)
        for _ in tqdm(range(num_variables)):
    ...
    def _train_bivariate(self):
        ...
        for effect in self._variables:
            ...
            for t in range(self._params["n_trials"]):
                ...
                log = train(ds_train, ds_test, params)
                results.append((effect, t, log))
    ...
    def _train_all_ntoone(self) -> dict[int, list[LogItems]]:
        ...
        for effect in self._variables:
            ...
            for t in range(self._params["n_trials"]):
                ...
                result = train(ds_train, ds_test, params)
                results.append((effect, t, result)) 
    ...
    def _select_var(
        ...
        self.best_model = self._log_models[k]
        return measures[k]
...
def train(
    ...
    if params['verbose']: pp(params["l_logdir"])
    ...
    for epoch in range(params["n_epoch"]):
        ...
        if (epoch+1) % params["interval_test"] == 0:
            x, y = next(iter(test_loader))  # Get all the test samples.
            model.train(False)
            with torch.no_grad():
                loss = model(x, y)
            model.train(True)
