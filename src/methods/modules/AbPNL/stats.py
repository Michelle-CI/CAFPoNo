...
def standardize(x, eps: float = 1e-5):
    return (x - x.mean(0)) / (x.std(0, ddof=0) + eps)


def g(
        x, bandwidth: float | None = None,
...
def test_th(
    ...
    return (th, varHSIC, mHSIC, alp, bet, stat)


def calc_HSIC_p(x, y) -> float:
